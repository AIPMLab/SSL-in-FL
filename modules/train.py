...
class TrainModule:
    def __init__(self, args, logger):
        self.args = args
        self.logger = logger
        self.net = NetModule(args)
        self.model = self.net.build_resnet9(decomposed=True)
        self.metrics = {
    ...
    def load_state(self, client_id):
        ...
        # 检查并构建优化器的权重
        if not hasattr(self.optimizer, 'weights') or len(self.optimizer.get_weights()) == 0:
            # 使用模型的可训练变量来构建优化器的权重
    ...
    def evaluate(self):
        ...
        # plt.ylabel('Ture label', fontsize=16)
        # plt.xlabel('Predicted', fontsize=16)
        # plt.tight_layout()
        #
        # # 保存混淆矩阵图片
        # output_dir = "C:\\Users\\PS\\Desktop\\FedMatch-main (3)\\FedMatch-main1\\outputs"  # 请指定你的保存路径
        # if not os.path.exists(output_dir):
        #     os.makedirs(output_dir)  # 如果目录不存在，创建它
        # cm_jpg_path = os.path.join(output_dir, f"confusion_matrix.jpg")
        # plt.savefig(cm_jpg_path, format='jpg', dpi=300)
    ...
    def uniform_average(self, updates):
        # 检查 updates 列表是否为空
        if len(updates) == 0:
            raise ValueError("No updates available to perform averaging.")

        # 提取权重和客户端大小
        client_weights = [u[0] for u in updates]
        client_sizes = [u[1] for u in updates]

        # 检查 client_weights 是否为空或其第一个元素为空
        if len(client_weights) == 0 or len(client_weights[0]) == 0:
            raise ValueError("client_weights is empty or not structured correctly.")
        ...
        for c in range(len(client_weights)):  # by client
            _client_weights = client_weights[c]
            for i in range(len(new_weights)):  # by layer
                new_weights[i] += _client_weights[i] * (client_sizes[c] / total_size)  # 按比例加权
