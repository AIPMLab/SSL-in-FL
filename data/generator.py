...
class DataGenerator:
    ...
    def load_dataset(self, dataset_id):
        temp = {}
        if self.args.dataset_id_to_name[dataset_id] == 'fashion_mnist':
            # 加载 Fashion-MNIST 数据集并转换为 RGB
            ...
            for dtype in ['train', 'test']:
                for image, target in temp[dtype]:
                    ...
                    print(f"Original image shape: {image.shape}")  # 打印图像转换前的形状

                    if len(image.shape) == 3 and image.shape[0] == 1:
                        image = np.squeeze(image, axis=0)  # 去除第一个通道，即 (1, 28, 28) -> (28, 28)
                        image = np.stack([image] * 3, axis=-1)  # 将灰度图像转换为 RGB 三通道 (28, 28) -> (28, 28, 3)

                    print(f"Converted RGB image shape: {image.shape}")  # 打印转换为 RGB 后的形状
                    x.append(image)
                    y.append(target)
    ...
    def save_task(self, data):
        assert data['x'].shape[-1] == 3, f"Unexpected number of channels before saving: {data['x'].shape[-1]}"
